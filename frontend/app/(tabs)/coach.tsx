  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={72}>
        <View style={styles.header}><View style={styles.bot}><Ionicons name="sparkles" size={22} color={colors.greenDark} /></View><View style={styles.headerCopy}><Text style={styles.title}>AI Nutrition Coach</Text><Text style={styles.status}>Personalized to today&apos;s progress</Text></View><View style={styles.online} /></View>
        <ScrollView contentContainerStyle={styles.messages} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" testID="coach-messages-list">
          ...
          {loading ? <View style={[styles.bubble, styles.coachBubble]}><ActivityIndicator color={colors.green} /></View> : null}
        </ScrollView>
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: colors.canvas }, flex: { flex: 1 }, header: { height: 76, paddingHorizontal: 20, flexDirection: "row", alignItems: "center", gap: 12, borderBottomWidth: 1, borderBottomColor: colors.line }, bot: { width: 46, height: 46, borderRadius: 17, backgroundColor: colors.greenSoft, alignItems: "center", justifyContent: "center" }, headerCopy: { flex: 1, gap: 3 }, title: { color: colors.ink, fontSize: 17, fontWeight: "900" }, status: { color: colors.muted, fontSize: 11 }, online: { width: 9, height: 9, borderRadius: 5, backgroundColor: colors.green }, messages: { padding: 20, paddingBottom: 25, gap: 12 }, bubble: { maxWidth: "84%", paddingHorizontal: 15, paddingVertical: 12, borderRadius: 20 }, coachBubble: { backgroundColor: colors.surface, alignSelf: "flex-start", borderBottomLeftRadius: 6 }, userBubble: { backgroundColor: colors.dark, alignSelf: "flex-end", borderBottomRightRadius: 6 }, bubbleText: { color: colors.ink, fontSize: 14, lineHeight: 21 }, userText: { color: "white" }, promptScroller: { maxHeight: 52 }, prompts: { paddingHorizontal: 16, gap: 8, alignItems: "center" }, prompt: { flexShrink: 0, height: 36, borderRadius: 18, paddingHorizontal: 13, backgroundColor: colors.greenSoft, justifyContent: "center" }, promptText: { color: colors.greenDark, fontSize: 11, fontWeight: "700" }, composer: { margin: 12, marginBottom: 5, minHeight: 56, paddingLeft: 16, paddingRight: 5, borderRadius: 28, backgroundColor: colors.surface, flexDirection: "row", alignItems: "center", gap: 8 }, input: { flex: 1, maxHeight: 92, color: colors.ink, fontSize: 14, paddingVertical: 12 }, send: { width: 46, height: 46, borderRadius: 23, backgroundColor: colors.peach, alignItems: "center", justifyContent: "center" }, disclaimer: { color: colors.muted, fontSize: 9, textAlign: "center", paddingBottom: 7 },
});
